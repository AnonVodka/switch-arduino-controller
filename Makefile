...
# without arguments.
all: swsh.hex usb-iface.hex test.hex visc.hex
...
flash-usb-iface: usb-iface.hex
	avrdude -v -p m16u2 -c $(PROGRAMMER) -P usb -U flash:w:$< -U lfuse:w:0xFF:m -U hfuse:w:0xD9:m -U efuse:w:0xF4:m -U lock:w:0x0F:m

restore-usb-iface: original.hex
	avrdude -v -p m16u2 -c $(PROGRAMMER) -P usb -U flash:w:$< -U lfuse:w:0xFF:m -U hfuse:w:0xD9:m -U efuse:w:0xF4:m -U lock:w:0x0F:m
...
original:
#   curl -o original.hex https://raw.githubusercontent.com/arduino/ArduinoCore-avr/master/firmwares/atmegaxxu2/arduino-usbserial/Arduino-usbserial-atmega16u2-Uno-Rev3.hex
	curl -o original.hex https://raw.githubusercontent.com/arduino/ArduinoCore-avr/master/firmwares/atmegaxxu2/UNO-dfu_and_usbserial_combined.hex
...
rebuild:
	rm -f swsh.hex usb-iface.hex src/*.o src/*.elf src/*.eep src/*/*.o src/*/*.elf src/*/*.eep
	make -C src/usb-iface clean
	$(MAKE) all
